...
/* Returns 1 if already locked and 0 if successfully locked, -1 if failed */

int isLocked(int err){
  if (err == EAGAIN){
    printf("It's locked\n");
    return 1;
  }
...
/* lock all cells we are trying in advance */
boolean lock_cells(grid *the_grid, int x, int y, int xdir, int ydir, char *word){
  boolean skip = False;
  int len = strlen(word);
  int i;
  for (i=0; (i<len) && (skip == False); i++){
    if (sem_trywait(&(the_grid->grid[x+i*xdir][y+i*ydir].cell_lock)) != 0){
      skip = True;

      /* Unlock all cells we've locked */
      //printf("Need unlocking2\n");
      unlock_cells(the_grid, x, y, xdir, ydir, i);

    }
    else{
      printf("The cell (%d, %d) is locked\n", (x+i*xdir), (y+i*ydir));
    }
    if (word[i] == the_grid->grid[x+i*xdir][y+i*ydir].letter){
      sem_post(&(the_grid->grid[x+i*xdir][y+i*ydir].cell_lock));
      printf("Letter agrees, unlock it\n");
    }
  }
  return skip;
}
...
void unlock_cells(grid *the_grid, int x, int y, int xdir, int ydir, int i){
  int j;
    printf("The size of the grid is: %d X %d\n", the_grid->rows, the_grid->cols);
  for (j=0; j<i; j++){
    if (sem_post(&(the_grid->grid[x+j*xdir][y+j*ydir].cell_lock)) == 0)
      printf("The cell (%d, %d) is unlocked\n", (x+j*xdir), (y+j*ydir));
  }
  //printf("%d cells unlocked\n", j);
}
...
grid_wordfits( grid *the_grid, char *word, int x, int y, int xdir, int ydir, int *count ) {
  ...
  if ((the_grid != NULL) && (word != NULL)) {
    ...
    if ( ((x + len*xdir) >= 0) && ((x + len*xdir) < the_grid->cols) &&
      ...
      skip=lock_cells(the_grid, x, y, xdir, ydir, word);

      if (skip == True){
        printf("Skip this try\n");
        return False;
      }
      ...
      for (i = 0; (fit == True) && (i < len); i++) {
        if ( word[i] == the_grid->grid[x + i*xdir][y + i*ydir].letter) {
          ...
          *count++;
          ...
          unlock_cells(the_grid, x, y, xdir, ydir, len);
          fit = False;
        }
      }
    }
