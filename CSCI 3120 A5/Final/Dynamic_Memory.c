...
int main(int argc, char **argv){
	...
	for (i=0; i<config.num_threads; i++){
		pthread_t p;
		pthread_create(&p, &attrib, thread_test, (void *)&(thread_nums[i]));
		threads[i] = p;
	}
...
void *get_memory(int size){
	...
	if (size <= 0){
		/* If the requested size is too small */
			printf("the size that is requested is too small! The program will not allocate any memory to this request.\n");
	...
	if (curr == NULL){
		...
		/* If there is enough space before the used_list head */
		int size_before_head = ((int)curr) - ((int)mem->mem_base) - MEMSIZE - NODESIZE;

		if (size_before_head >= size){

			Node *new = (Node *)((int)mem->mem_base + MEMSIZE);
			if (new != NULL){
				new->data = (void *)((int)mem->mem_base + MEMSIZE + NODESIZE);
				new->prev = NULL;
				new->next = curr;
				new->size = size;
				new->pointers = 1;
				...
				printf("Some thing wrong when assign memory.\n");
				exit(EXIT_FAILURE);
			}
...
void *grow_memory(int size, void *p){
	...
	/* If the size is less than or equal to 0, release the memory */
	if (size <= 0){
		printf("Now release the memory.\n");
	...
	/* If the size is less than the original size, just update the size */
	if (size < p_node->size && size > 0){
		p_node->size = size;
		if (sem_post(&mem_lock) == -1){
			perror(strerror(errno));
			exit(EXIT_FAILURE);
	...
	if (p_node != NULL){
		...
		int requested_size = size;	//the total size it will have if the memory is growed
		...
		/* If not, try to find another location for if */
			...
			if (new_memory_patition != NULL){
				...
				p_node = (Node *)(new_memory_patition - NODESIZE);
			}
              		...
              		case 's':
              			...
              			if (i < argc){
              				...
              				if (atoi(argv[i]) > 0 && (atoi(argv[i]) >= MINMEMSIZE)){
              					...
              					printf("The memory size you entered is too small, system will use %d bytes.\n", MINMEMSIZE);
              					config->size = MINMEMSIZE;
              				}
              				else{
...
void *thread_test(void *pv){
	...
	int *thread_num_pointer = (int *)pv;
	int thread_num = *thread_num_pointer;
	...
	printf("thread: %d; p_node: %p; p_node->next: %p\n", thread_num, p_node, p_node->next);
	printf("p1_node: %p; p1_node->prev: %p;p1_node->next: %p.\n", p1_node, p1_node->prev, p1_node->next);
	printf("thread: %d; p2_node: %p; p2_node->prev: %p;p2_node->next: %p.\n", thread_num, p2_node, p2_node->prev, p2_node->next);
	...
	p1 = NULL;
	p1_node = NULL;
	printf("thread: %d; p1_node: %p\n", thread_num, p1_node);
	printf("thread: %d; p_node->next: %p\n", thread_num, p_node->next);
	printf("thread: %d; p2_node->prev: %p\n", thread_num, p2_node->prev);
	void *p3 = get_memory(48);
	Node *p3_node = (Node *)((int)p3 - NODESIZE);
	printf("thread: %d; p3_node: %p; p3_node->prev: %p;p3_node->next: %p.\n", thread_num, p3_node, p3_node->prev, p3_node->next);
	printf("thread: %d; p_node->next: %p; p2_node->prev: %p\n", thread_num, p_node->next, p2_node->prev);
	printf("thread: %d; grow p3 memory.\n", thread_num);
	printf("thread: %d; The address of p3 before: %p; size of p3 before: %d; offset of p3 before: %p.\n", thread_num, p3, p3_node->size, p3_node);
	...
	p3 = grow_memory(92, p3);
	/* Update pointer */
	p3_node = (Node *)((int)p3 - NODESIZE);

	printf("thread: %d; The address of p3 now: %p; size of p3 now: %d; offset of p3 now: %p.\n", thread_num, p3, p3_node->size, p3_node->data);
	release_memory(p3);
	//release_memory(p);
	release_memory(p2);
	p3_node = NULL;
